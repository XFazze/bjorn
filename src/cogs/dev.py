...
class dev(commands.Cog):
    ...
    async def sync(self, ctx: commands.Context):
        ...
        await ctx.interaction.response.send_message(embed=discord.Embed(title=f"Synced!", color=0x00FF42), ephemeral=True)

    @commands.hybrid_command(name="devchannel", description="Creates a dev channel")
    @permissions.admin()
    async def devchannel(self, ctx: commands.Context):
        category_search = [category for category in ctx.guild.categories if category.name == os.getenv(
