...
class activePeople(general.Bjorn_cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.active_role_id = int(os.environ["LOADING_ACTIVE_ROLE_ID"])
        self.inactive_role_id = int(os.environ["LOADING_INACTIVE_ROLE_ID"])
    ...
    async def on_voice_state_update(self, member, before, after):
        if (
            ...
            or member.guild.id != self.loading_id
            or not member
            or member.bot
        ):
            return
