...
class Database(general.Database):
    ...
    def how_flamed(self, discord_id: int):
        res = self.cursor.execute(f"SELECT discord_id, money, timestamp FROM pot").fetchall()
        pot = 0
        for i in res:
    ...
    def add_flamed(self, discord_id:int):
        tiden = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        discord_id = str(discord_id)
        self.cursor.execute(
            f"INSERT INTO pot (discord_id, money, timestamp) VALUES (?, ?, ?)", (discord_id, pot_value, tiden)
        )
